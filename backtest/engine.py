...
def _compute_phase1_cache_key(tickers: list, has_quarterly: bool) -> str:
    ...
    key_parts = "|".join([
        ...
        str(_cfg.FACTOR_WEIGHTS.momentum),
        str(_cfg.FACTOR_WEIGHTS.quality),
        str(_cfg.FACTOR_WEIGHTS.value),
        ",".join(sorted(tickers)),
    ])
...
# ── 部分利確（Partial Exit）設定 ────────────────────────────────────
# 含み益が PARTIAL_EXIT_R_MULT × R に達したら PARTIAL_EXIT_RATIO を利確
# 残りは SL をブレイクイーブンに移動して trail 継続
# P/L = ratio × 第1出口 P/L + (1-ratio) × 第2出口 P/L
PARTIAL_EXIT_ENABLED: bool  = False  # 検証済み: ベースライン比 -56% のため無効化
PARTIAL_EXIT_R_MULT: float  = 2.0   # 2R（entry-sl の2倍）到達でトリガー
PARTIAL_EXIT_RATIO: float   = 0.5   # 50%を利確
# ─────────────────────────────────────────────────────────────────────
...
# 最小ATRフィルター: ATR/entry_price がこの値未満の低ボラ銘柄をスキップ
# SL = ATR×2 なので MIN_ATR_PCT=0.015 は SL<3% の銘柄を除外することに相当
# 分析: SL<3%帯は期待値0.59%、SL3-5%帯は1.03% → 低ATR銘柄が足を引っ張っている
MIN_ATR_PCT: float = 0.0     # 0=無効。検証済み(2026-03-08): 1.5%設定は勝率+4.5pt改善も総リターン-34pt悪化→不採用
...
def _run_phase2(
    ...
    sort_by_risk_adj: bool = False,   # Case D: リスク調整スコア順でエントリー優先度を変更
    sb_sl_mult: float | None = None,  # Case E: STRONG_BUY 専用 SL 倍率 (例: 2.5)
) -> list[TradeResult]:
    """Phase2: シグナル候補を日付順にポートフォリオシミュレーション。

    sb_trail_mult を指定することで STRONG_BUY のトレーリング幅を上書きできる。
    sort_by_risk_adj=True: スコア / ATR比率 の降順でエントリー優先度を変更 (Case D)
    sb_sl_mult: STRONG_BUY の SL 倍率を上書き (例: 2.5 → ATR×2.5) (Case E)
    Phase1 の結果（signal_candidates）を使い回して複数パラメータを比較可能。
    """
    all_trades: list[TradeResult] = []
    active_positions: dict[str, dict] = {}

    # Case D: リスク調整スコア順（スコア÷ATR比率）でソート
    # 通常はスコア降順。sort_by_risk_adj=True の場合は「同リスクでより多く買える」銘柄を優先
    if sort_by_risk_adj:
        def _risk_adj_key(c: dict) -> float:
            atr_ratio = c["atr"] / c["entry_price"] if c["entry_price"] > 0 else 0.08
            return -c.get("score", 0.0) / atr_ratio if atr_ratio > 0 else 0.0
        signal_candidates = sorted(signal_candidates, key=lambda c: (c["date"], _risk_adj_key(c)))
    # else: signal_candidates はすでに (date, -score) 順でソート済み
    ...
    for date_str in sorted_dates:
        ...
        for c in day_candidates:
            ...
            atr = c["atr"]
            # Case E: STRONG_BUY は SL 倍率を上書き（wider SL で早期損切りを減らす）
            _sl_m = (sb_sl_mult if sb_sl_mult is not None and c["signal"] == "STRONG_BUY"
                     else sl_multiplier)
            atr_sl   = entry_price - atr * _sl_m
            hard_sl  = entry_price * (1.0 + hard_stop_pct)
            sl_price = max(atr_sl, hard_sl)
            if sl_price <= 0:
                sl_price = entry_price * 0.90

            # 最小ATRフィルター: 低ボラ銘柄をスキップ (SL<3% = 期待値低い帯)
            if MIN_ATR_PCT > 0 and entry_price > 0 and atr / entry_price < MIN_ATR_PCT:
                continue
...
def _simulate_trade(
    ...
    # ── 部分利確の状態管理 ──
    r_distance = entry_price - sl_price  # 1R = entry - SL
    partial_done = False
    partial_pl_pct = 0.0
    partial_sl = sl_price  # 部分利確後に BE に移動する SL
    ...
    for idx in range(len(future_df)):
        ...
        # ── 部分利確チェック（2R到達で50%利確、残りSLをBEへ） ──
        if PARTIAL_EXIT_ENABLED and not partial_done and r_distance > 0:
            target_2r = entry_price + PARTIAL_EXIT_R_MULT * r_distance
            if high >= target_2r:
                partial_done = True
                partial_pl_pct = (target_2r - entry_price) / entry_price
                # 残り50%のSLをブレイクイーブン（entry価格）に引き上げ
                partial_sl = entry_price

        # 現在有効なSLを決定（部分利確後はBE SLを使用）
        effective_sl = partial_sl if partial_done else sl_price

        # SL判定（ギャップダウン対応: 始値がSL以下なら始値で約定）
        open_price_row = float(row["Open"]) if "Open" in row.index else float(row["Low"])
        if low <= effective_sl or open_price_row <= effective_sl:
            exit_price_sl = min(effective_sl, open_price_row)
            second_pl = (exit_price_sl - entry_price) / entry_price
            final_pl = (PARTIAL_EXIT_RATIO * partial_pl_pct + (1 - PARTIAL_EXIT_RATIO) * second_pl
                        ) if partial_done else second_pl
            result_label = "partial_sl" if partial_done else "sl_hit"
            return TradeResult(
                ...
                result=result_label,
                pl_pct=final_pl,
                sl_price=sl_price, tp_price=partial_pl_pct if partial_done else 0.0, score=score,
            )
        ...
        if holding_days > 5 and trail_level >= entry_price and close <= trail_level:
            second_pl = (trail_level - entry_price) / entry_price
            final_pl = (PARTIAL_EXIT_RATIO * partial_pl_pct + (1 - PARTIAL_EXIT_RATIO) * second_pl
                        ) if partial_done else second_pl
            result_label = "partial_trail" if partial_done else "trailing_stop"
            return TradeResult(
                ticker=ticker, signal=signal, signal_date=signal_date,
                entry_date=entry_date, entry_price=entry_price,
        ...
        if _use_timeout:
            ...
            for threshold_days, min_profit in STAGED_TIMEOUT:
                if holding_days == threshold_days and pl_pct < min_profit:
                    ...
                    result_label = f"partial_to{threshold_days}d" if partial_done else f"timeout_{threshold_days}d"
                    return TradeResult(
                        ticker=ticker, signal=signal, signal_date=signal_date,
                        entry_date=entry_date, entry_price=entry_price,
        ...
        if holding_days >= _max_hold:
            second_pl = (close - entry_price) / entry_price
            final_pl = (PARTIAL_EXIT_RATIO * partial_pl_pct + (1 - PARTIAL_EXIT_RATIO) * second_pl
                        ) if partial_done else second_pl
            result_label = f"partial_to{_max_hold}d" if partial_done else f"timeout_{_max_hold}d"
            return TradeResult(
                ticker=ticker, signal=signal, signal_date=signal_date,
                entry_date=entry_date, entry_price=entry_price,
                exit_date=date_str, exit_price=close,
    ...
    second_pl = (last_close - entry_price) / entry_price
    final_pl = (PARTIAL_EXIT_RATIO * partial_pl_pct + (1 - PARTIAL_EXIT_RATIO) * second_pl
                ) if partial_done else second_pl
    result_label = f"partial_to{_max_hold}d" if partial_done else f"timeout_{_max_hold}d"
    return TradeResult(
